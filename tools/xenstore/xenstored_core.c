...
static void setup_structure(void)
{
	...
	if (tdb_ctx) {
		...
		*/
        ...
        manual_node("/", "local");
        manual_node("/local", "domain");
        manual_node("/local/domain", "0");
        manual_node("/local/domain/0", NULL);
        manual_node("/local/domain/0", "control");
        manual_node("/local/domain/0/control", "shutdown");
        manual_node("/local/domain/0/control/shutdown", NULL);
        manual_node("/local/domain/0/control", "feature-poweroff");
        manual_node("/local/domain/0/control/feature-poweroff", NULL);
        manual_node("/local/domain/0/control", "feature-reboot");
        manual_node("/local/domain/0/control/feature-reboot", NULL);
        manual_node("/local/domain/0/control", "feature-halt");
        ...
        manual_node("/local/domain/1/device/vif", "0");
        manual_node("/local/domain/1/device/vif/0", "backend-id");
        manual_node("/local/domain/1/device/vif/0/backend-id", NULL); 
        manual_node("/local/domain/0", "backend");
        manual_node("/local/domain/0/backend", "vif");
...
int main(int argc, char *argv[])
{
   ...
   // struct xs_handle *xsh;
	...
	while ((opt = getopt_long(argc, argv, "DEU:F:HNPS:t:T:RLVW:", options,
				  NULL)) != -1) {
		switch (opt) {
		case 'D':
			no_domain_init = true;
