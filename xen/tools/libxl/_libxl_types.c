...
void libxl_sched_params_copy(libxl_ctx *ctx, libxl_sched_params *dst, libxl_sched_params *src)
{
    GC_INIT(ctx);
...
void libxl_vcpu_sched_params_copy(libxl_ctx *ctx, libxl_vcpu_sched_params *dst, libxl_vcpu_sched_params *src)
{
    GC_INIT(ctx);
    dst->sched = src->sched;
...
void libxl_device_usbctrl_copy(libxl_ctx *ctx, libxl_device_usbctrl *dst, libxl_device_usbctrl *src)
{
    GC_INIT(ctx);
    dst->type = src->type;
    dst->devid = src->devid;
...
void libxl_device_usbdev_copy(libxl_ctx *ctx, libxl_device_usbdev *dst, libxl_device_usbdev *src)
{
    GC_INIT(ctx);
    dst->ctrl = src->ctrl;
    dst->port = src->port;
...
void libxl_usbctrlinfo_copy(libxl_ctx *ctx, libxl_usbctrlinfo *dst, libxl_usbctrlinfo *src)
{
    GC_INIT(ctx);
    dst->type = src->type;
    dst->devid = src->devid;
...
libxl_enum_string_table libxl_device_model_version_string_table[] = {
    ...
    { .s = "none", .v = LIBXL_DEVICE_MODEL_VERSION_NONE },
    { NULL, -1 },
};
